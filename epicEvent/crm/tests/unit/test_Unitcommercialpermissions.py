...
def commercial_user(db):    
    User = get_user_model()
    return User.objects.create_user(username='commercial', password='testpass123', role='commercial')
...
def test_commercial_create_client_unit(mocker, commercial_user, api_client, client_data):
    
    mock_client_instance = Mock(id=1, **client_data)
    
    mocked_serializer_instance = mocker.Mock(spec=ClientSerializer)
    mocked_serializer_instance.data = {'id': 1, **client_data}
    mock_save = mocker.patch('crm.serializers.ClientSerializer.save', return_value=mocked_serializer_instance)
...
def test_commercial_update_client_unit(mocker, commercial_user, api_client, client_data):
    
    mock_client = Mock(id=1, **client_data, sales_contact=commercial_user)    
    
    mocker.patch('crm.views.ClientViewSet.get_object', return_value=mock_client)    
    ...
    mocker.patch('crm.views.ClientViewSet.get_serializer', return_value=mock_serializer)   
    api_client.force_authenticate(user=commercial_user)
    
    updated_data = {'full_name': 'Updated Client'}
   
    response = api_client.patch(reverse('client-detail', args=[mock_client.id]), updated_data)    
    assert response.status_code == status.HTTP_200_OK
    mock_serializer.is_valid.assert_called_once()
    mock_serializer.save.assert_called_once()

def test_support_user_read_only_access_unit(mocker, support_user, api_client, client_data, commercial_user):
    ...
    response = api_client.get(reverse('client-detail', args=[mock_client.id]))
...
def test_management_user_read_only_access_unit(mocker, management_user, api_client, client_data):
...
def test_authenticated_user_view_client_details_unit(mocker, api_client, client_data, commercial_user):
...
def test_unauthenticated_user_actions_unit(mocker, api_client, client_data):
    ...
    response = api_client.delete(reverse('client-detail', args=[mock_client.id]))
    assert response.status_code in [status.HTTP_401_UNAUTHORIZED, status.HTTP_403_FORBIDDEN]



def test_commercial_user_view_unsigned_contracts_unit(mocker, commercial_user, api_client):
    ...
    mock_method = mocker.patch(
        ...
        return_value=Response(mock_response_data, status=status.HTTP_200_OK)
    )
