...
def test_management_cannot_delete_client(api_client, mock_client_object, management_user):
    api_client.force_authenticate(user=management_user)
...
def test_management_cannot_update_client(api_client, mock_client_object, management_user):
    api_client = APIClient()
    api_client.force_authenticate(user=management_user)
...
def test_management_can_read_all_users(mocker):
    api_client = APIClient()
    User = get_user_model()
    management_user = User.objects.create_user(username='manager', password='password', role='management')
    api_client.force_authenticate(user=management_user)
...
def test_management_can_update_user(mocker):
    ...
    user_to_update = User.objects.create_user(username='existinguser', password='password', role='commercial')
...
def test_management_can_delete_user(mocker):
    ...
    management_user = User.objects.create_user(username='manager', password='password', role='management')
    api_client.force_authenticate(user=management_user)
    user_to_delete = User.objects.create_user(username='deleteuser', password='password', role='user_role')      
...
def test_management_can_read_all_contracts(mocker):
    api_client = APIClient()
    mocked_user = mocker.patch('rest_framework.request.Request.user', create=True)
    mocked_user.return_value.is_authenticated = True
    mocked_user.return_value.role = 'management'

    
    mocked_queryset = mocker.patch('crm.views.ContractViewSet.get_queryset')
...
def test_management_cannot_create_events():
    ...
    event_data = {
        ...
        "attendees": 25,
        "notes": "Hello Sed convallis tellus at urna iaculis ultrices. Aenean placerat orci mauris, nec rhoncus nisl egestas nec. Phasellus at neque blandit, eleifend massa eget, euismod neque. Nulla eu feugiat quam, quis tincidunt turpis. Duis ut mauris at felis auctor ullamcorper sit amet vitae nisl. In tristique orci in congue posuere. Ut dapibus justo non lacus interdum, non elementum libero viverra. Nullam ac tortor euismod, vestibulum nulla elementum, lobortis tortor. Nullam sit amet dui sit amet est bibendum pulvinar eget in ipsum. Integer iaculis ex sit amet lorem malesuada sollicitudin.",
        "contract": 7,
        "client_name": 3
    }
