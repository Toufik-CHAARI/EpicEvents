...
def mock_contract_object(mocker):
    ...
    mock_contract.id = 1
    mocker.patch(
        "crm.models.Contract.objects.get", return_value=mock_contract
    )
...
def mock_event_object(mocker, support_user):
    ...
    mocker.patch(
        "crm.models.Event.objects.get", return_value=mock_event
    )
    return mock_event
...
def test_support_cannot_delete_client(
    api_client, mock_client_object, support_user
):
    api_client.force_authenticate(user=support_user)
...
def test_support_cannot_delete_contract(
    api_client, mock_contract_object, support_user
):
    api_client.force_authenticate(user=support_user)
...
def test_support_cannot_update_contract(
    ...
    api_client.force_authenticate(user=support_user)
    response = api_client.patch(
        reverse("contract-detail", args=[mock_contract_object.id]), {}
    )
    assert response.status_code == status.HTTP_403_FORBIDDEN
...
def test_support_user_can_update_assigned_event():
    ...
    support_user = User.objects.create_user(
        username="support", password="password", role="support"
    )
    api_client.force_authenticate(user=support_user)
...
@pytest.mark.django_db
def test_support_user_cannot_update_unassigned_event(
    api_client, support_user
):
    client_instance = Client.objects.create(
        full_name="Test Client",
        email="testclient@example.com",
