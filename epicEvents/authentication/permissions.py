...
class IsCommercialUser(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        if (
            ...
            or request.user.role != "commercial"
        ):
            return False
...
class IsSupportUser(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        return (
            obj.support_contact == request.user
            if view.action in ["update", "partial_update"]
            else True
        )
...
class IsCommercial(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        return (
            obj.contract.is_signed
            and obj.contract.sales_contact == request.user
        )
